...
def gen_loo_plots(cfname, gfname):
    ...
    with open(cfname) as cfile:
        reader = csv.DictReader(cfile)
        for row in reader:
            nrepeats = len(ast.literal_eval(row['read']))
            dframe['nrepeats'] = nrepeats
            if row['consensus'] != '':
                dframe['consensus_edit'].append(row['consensus_edit'])
                dframe['consensuss_r_edit'].append(row['consensus_r_edit'])
            else:
                dframe['conseus_edit'] = None
                dframe['consensus_r_edit'] = None
